...
def _prepare_stream(stream, output_file,
    ...
    if freq_domain and normalize is None:
        stream.fft()
    if whitening:
    ...
    if freq_domain and normalize is not None:
            stream.fft()
    if output_file:
...
def prepare(data, stations, t1, t2, component='all', use_floating_stream=True, pool=None, max_preload=5, **kwargs):
    """
...
#            data2 = FloatingStream(data, t1, station2, 0, component=comp2, period=period)
        ...
        for t in timegen(t1, t2, period):
            if len(xcorr) > 0 and (t - period).date != t.date and (
                    (period > 3600 and t.getJulday() == 1) or period <= 3600):
            ...
            try:
                ...
                log.debug('Could not load data for %s %s' % (str(correlation), t))
                continue
...
def plotXcorrs(data, correlations, t1, t2, filters=None, start=None, end=None, select=None, plot_overview=True, plot_years=True,
...
def stretch(stream, reftr, start=None, end=None, relative='starttime', str_range=0.1, nstr=100, time_windows=None, single_side=False, left_side=False):
