...
#        infoline += ';' + eventdict['magnitude_type']
        # create event-folder
        eventdir = os.path.join(datapath, str(day.year))
...
#        quakefout.seek(0, 2)
        ...
        for station in arclink_stations:
            ...
            datafout = os.path.join(eventdir, "%s_%s.mseed" % (station, eventid))
            if os.path.isfile(datafout):
                print 'Data file for day %s from %s exists, skip...' \
                                                           % (eventid, station)
                continue
...
#                quakefout.flush()  # (\\label{lst:qcend})
        ...
        for net, sta, loc, cha, stationlat, stationlon in avail:
            ...
            irisfn = "%s_%s.mseed" % (station, eventid)
            irisfnfull = os.path.join(datapath, irisfn)
            if options.debug:
